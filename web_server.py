...
from src.minic.interpreter import RuntimeError as MiniCRuntimeError
from src.minic.lexer import LexicalError
from src.minic.parser import ParseError
from src.minic.pipeline import VALID_STAGES, run_stage
...
class MiniCRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            output = run_stage(source, stage)
            self._send_json({"ok": True, "output": output})
        except (LexicalError, ParseError, SemanticError, MiniCRuntimeError, ValueError) as error:
